...
public class BuilderSceneBattle implements BuilderScene {
	private String path = "..\\masterImages\\backGround1.png";
	private String craftFixedWall = "..\\masterImages\\bloco1.png";
	private String craftMobileWall = "..\\masterImages\\blocoQuebravel.png";
	...
	public int howManyBombers(ArrayList<Bomber> bombers) {
		int AllWalls = Const.NUM_MOBILE_WALLS;
		int numMasterBombers = 0;//Contador de números de bombermans
		x = y = 0;
		
		//Conta o número de bombermans
		for (int i = 0; i < bombers.size(); i++) {
			Bomber br = bombers.get(i);
			if(br instanceof MasterBomber) {
				numMasterBombers++;
			}
		}
		return numMasterBombers;
	}
	...
	public void initMobileWalls(Scene scene, ArrayList<Bomber> bombers) {
		...
		for(int i = 0; i < Const.NUM_ALL_WALLS; i++) {
			y = (i*Const.TAM_WALL);
			for (int j = 0; j < Const.NUM_ALL_WALLS; j++) {
				x = (100 + ( j * Const.TAM_WALL ));
				boolean passed = true;
	...
	public void initItemList(Scene scene) {
		...
		while(aux < Const.NUM_ITEM_LIST_TO_ADD) {
			...
			if(chosenWall > 63) {//as paredes m[oveis estáo acima do indice 63
